...
function ENT:Initialize()
	net.Receive("CTG_Characters_WardrobeFrame", function()
		CTG_Characters_OpenFrame("CTG_Characters_Wardrobe")
	end)
end
...
function ENT:Draw()
	...
	if string.len(string.Trim("Vêtements")) < 1 then return end
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 300 then
		...
		cam.Start3D2D(self:GetPos() + Vector(0,0,80), a , 0.08) 
			...
			draw.SimpleText("Vêtements","DermaLarge",0,-40, white , 1 , 1) 
		cam.End3D2D() 
	end
end